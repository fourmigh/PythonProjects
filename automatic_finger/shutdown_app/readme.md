...
基于 **"视觉识别（OCR）给坐标 → 人形鼠标点击"** 的 Windows 自动关机工具。
...
4. **OCR 定位"关机"**：rapidocr 在弹框区域识别三个菜单项；用**睡眠行与重启行的中点**作为"关机"行的点击中心（比单条文字 OCR 中心更抗抖动）。
5. **人形鼠标移动 + 点击**：沿贝塞尔曲线移动到位，点击。
6. **点击后校验 + 安全重试**：点击后等待 2 秒，重新 OCR 弹框区域：
   - 弹框已关闭 → 点击生效，系统正在关机，流程成功；
   - 弹框仍在 → 视为未点中，在"关机"行内做 y 偏移 `[0, +8, -8, +16, -16]` 重试（**约束在"关机"行内，不会点到睡眠/重启**）；
   - 5 次仍失败 → 保存一张 `失败截图.png` 到 exe 旁并安全中止（绝不点其他入口）。
...
### 全鼠标方案（无键盘）
目标环境键盘注入被拦截：`SendInput`/`keybd_event` 发送 Win 键等均无效（返回 0 / 无效果），而鼠标注入（pyautogui / SendInput）完全正常。因此流程设计为纯鼠标：点开始 → 点电源 → OCR 定位 → 点击关机。
...
（命名来源：最初面向"需要规避检测的自动化"场景，拟人曲线+抖动+随机延迟使其不易被识别为机器操作；本项目亦使关机流程看起来像人手点击。）
...
1. 双击 `关机助手.exe`，UAC 弹窗点"是"（程序需要管理员权限才能点击开始菜单/关机）。
2. 界面操作：
   - **干跑测试**：完整执行"点开始 → 点电源 → OCR 定位 → 移动鼠标到'关机'行"，但**不点击**，用于安全验证；
   - **开始自动关机**：正式执行，移动到"关机"行并点击，系统关机；
   - 点击后进入 5 秒倒计时。
3. **急停**：把鼠标甩到屏幕左上角（`pyautogui.FAILSAFE`），立即中止。

### 自动化验证
```bat
关机助手.exe --auto-dry
```
启动后自动执行一次干跑，用于无人值守验证。
